...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run task-free continual-learning baselines for CAPE protocols.")
    ...
    parser.add_argument("--output-root", default="results/continual_baselines")
    ...
    print(f"[Baseline] project_root={PROJECT_ROOT}")
    print(f"[Baseline] tasks={task_sequence}")
    print(f"[Baseline] methods={methods}")
    print(f"[Baseline] eval_split={args.eval_split}")
    print(f"[Baseline] device={args.device}; epochs={args.epochs}; batch_size={args.batch_size}")
    print(
        f"[Baseline] num_workers={args.num_workers}; "
        f"pin_memory={not args.no_pin_memory}; amp={args.amp}"
    )

    for method in methods:
        output_dir = os.path.join(args.output_root, method)
        print(f"\n[Baseline] Running {method} -> {output_dir}")
        ...
        trainer.run()
        print(f"[Baseline] Finished {method}")
