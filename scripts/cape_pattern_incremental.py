...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run CAPE pattern-incremental training.")
    parser.add_argument("--metadata", default="data/cape_metadata.csv")
    parser.add_argument("--tasks", default=DEFAULT_GENERATOR_TASKS, help="Comma-separated task ids.")
    ...
    parser.add_argument("--output-dir", default="results/cape_generator_e20")
    ...
    parser.add_argument("--eval-every-epoch", dest="eval_every_epoch", action="store_true", default=True)
    ...
    parser.add_argument(
        "--eval-split",
        default="test",
        help="Split used for post-task evaluation. Use val for AV-Deepfake1M because its public metadata has train/val labels.",
    )
    ...
    print(f"[CAPE] project_root={PROJECT_ROOT}")
    print(f"[CAPE] metadata={args.metadata}")
    print(f"[CAPE] tasks={args.tasks}")
    print(f"[CAPE] output_dir={args.output_dir}")
    print(f"[CAPE] eval_split={args.eval_split}")
    print(f"[CAPE] device={args.device}; batch_size={args.batch_size}; epochs={args.epochs}")
    print(f"[CAPE] lr={args.lr}; replay_capacity={args.replay_capacity}")
    print(
        f"[CAPE] num_workers={args.num_workers}; "
        f"pin_memory={not args.no_pin_memory}; amp={args.amp}"
    )
    if args.max_train_batches or args.max_eval_batches:
        print(f"[CAPE] debug limits: train={args.max_train_batches}; eval={args.max_eval_batches}")
    if args.eval_every_epoch:
        print(f"[CAPE] eval_every_epoch=True; save_best={args.save_best}; best_metric={args.best_metric}")
    print(f"[CAPE] backbone_ckpt={args.backbone_ckpt or 'none'}")
    ...
    if trainer.model.backbone_load_report:
        print(f"[CAPE] Backbone load: {trainer.model.backbone_load_report}")
    history = trainer.run()
    ...
    print(f"[CAPE] History saved to {history_path}")
    print(f"[CAPE] Summary saved to {summary_path}")
